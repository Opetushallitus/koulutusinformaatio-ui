const production = {
    "konfo-backend.base-url" : "/",
    "konfo-backend.search.koulutukset" : "/konfo-backend/search/koulutukset",
    "konfo-backend.search.organisaatiot" : "/konfo-backend/search/organisaatiot",
    "konfo-backend.koulutus" : "/konfo-backend/koulutus",
    "konfo-backend.organisaatio" : "/konfo-backend/organisaatio"
};

const development = {
    "konfo-backend.base-url" : "http://localhost:3006",
    "konfo-backend.search.koulutukset" : "http://localhost:3006/konfo-backend/search/koulutukset",
    "konfo-backend.search.organisaatiot" : "http://localhost:3006/konfo-backend/search/organisaatiot",
    "konfo-backend.koulutus" : "http://localhost:3006/konfo-backend/koulutus/",
    "konfo-backend.organisaatio" : "http://localhost:3006/konfo-backend/organisaatio/"
};

export {production, development}