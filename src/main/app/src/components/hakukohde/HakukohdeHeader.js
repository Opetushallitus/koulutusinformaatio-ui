...
class HakukohdeHeader extends Component {
  ...
  render() {
    const link = `/koulutus/${this.props.komoOid}?haku=${encodeURIComponent(
      this.props.hakuStore.createHakuUrl
    )}&lng=${l.getLanguage()}`;
    return (
      <div className="compare ">
        <p className="hakukohde-header-second">
          Hakukohteen tiedot ja valintaperusteet
        </p>
        <h1 id="hakukohde-header" className="d-flex justify-content-between">
          <Link
            id="hakukohde-koulutus-link"
            to={link}
