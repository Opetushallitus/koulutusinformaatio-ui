...
class Avoin extends Component {
  ...
  parseKuvaus() {
    if (
      this.props.result &&
      this.props.result.kuvausKomoto &&
      this.props.result.kuvausKomoto.SISALTO
    ) {
  ...
  parseInfoBoxFields() {
    ...
    fields.push([
      ...
      this.props.result.opintopisteet ? this.props.result.opintopisteet : '',
    ]);
    fields.push([
      t('koulutus.koulutusohjelma'),
      l.localize(this.props.result.koulutusohjelma),
    ]);
    fields.push([
      t('koulutus.opetuskielet'),
      this.props.result.opetuskielis
        ? this.props.result.opetuskielis.map((kieli) => l.localize(kieli))
        : '',
    ]);
    fields.push([
      t('koulutus.suoritustavat'),
      this.props.result.opetusPaikkas
        ? this.props.result.opetusPaikkas.map((paikka) => l.localize(paikka))
        : '',
    ]);
    fields.push([
      t('koulutus.toimipiste'),
      this.props.result.toimipiste ? this.props.result.toimipiste : '',
    ]);
    fields.push([
      t('koulutus.ajoitus'),
      this.props.result.ajoitus ? this.props.result.ajoitus : '',
    ]);
    fields.push([
      t('koulutus.opetusajat'),
      this.props.result.opetusAikas
        ? this.props.result.opetusAikas.map((aika) => l.localize(aika))
        : '',
    ]);
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        <SlideDropDown
          title="Mihin koulutus antaa valmiudet?"
          text={true}
        ></SlideDropDown>
        <SlideDropDown title="Mitä voin opiskella?" text={true}></SlideDropDown>
        <SlideDropDown
          title="Mitä tutkintoja voin suorittaa?"
          text={true}
        ></SlideDropDown>
        <SlideDropDown
          title="Mitä jatko-opintomahdollisuuksia on?"
          text={true}
        ></SlideDropDown>
        <OppilaitosList
          oid={this.props.oid}
