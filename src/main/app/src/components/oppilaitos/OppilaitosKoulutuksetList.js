...
class OppilaitosKoulutuksetList extends Component {
  ...
  async componentWillReceiveProps(nextProps) {
    if (nextProps.name !== this.state.name) {
      this.setState({
        name: nextProps.name.indexOf(',')
          ? nextProps.name.split(',')[0]
          : nextProps.name,
      });
      await this.setKoulutukset();
    }
  ...
  setKoulutukset() {
    ...
    fetch(
      `https://hahtuvaopintopolku.fi/konfo-backend/search/toteutukset?keyword=${name}&size=20&page=1`,
      {
        method: 'GET',
      }
    )
  ...
  render() {
    ...
    return (
      <div className="col-12" id="oppilaitos-koulutukset-container">
        <h1>{`Koulutukset (${koulutukset &&
          koulutukset.count &&
          koulutukset.count})`}</h1>
        {koulutukset &&
          koulutukset.result &&
          koulutukset.result.map((r) => (
            <HakutulosBox
              ...
              link={`/koulutus/${r.koulutusOid}?haku${encodeURIComponent(
