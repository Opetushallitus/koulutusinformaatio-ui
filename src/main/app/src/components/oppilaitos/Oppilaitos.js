...
class Oppilaitos extends Component {
      ...
      if (row.www) {
        return (
          <a href={row.www}>
            ...
            {this.props.t('oppilaitos.verkkosivu')}
          </a>
        );
      }
    }
  ...
  render() {
    ...
    const actualOppilaitos =
      this.state.oppilaitos !== undefined
        ? l.localize(this.state.oppilaitos.nimi, '', 'fi')
        : 'Nimi';
    const emailAddress =
      this.state.oppilaitos !== undefined
        ? this.getEmailFromYhteystiedot()
        : '';
    const ratingStars = [
      { star: 1 },
      { star: 1 },
