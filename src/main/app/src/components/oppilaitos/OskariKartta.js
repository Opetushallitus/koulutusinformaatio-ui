...
class OskariKartta extends Component {
    ...
    setLocation() {
        ...
        if(this.state.channel) {
            ...
            console.log("Odottamaton virhe. Karttapalvelun channel puuttuu " + this.props.urlStore.urls.url('kartta.base-url'));
        }
    }
    ...
    initMap() {
        ...
        channel.handleEvent(
            'SearchResultEvent',
            function(data) {
                if(data.success && data.result && data.result.locations && data.result.locations.length > 0) {
                    let location = data.result.locations[0];
                    data.result.locations.forEach(loc => {
                        if(loc.region.toLowerCase() === _this.props.postitoimipaikka.toLowerCase() ||
