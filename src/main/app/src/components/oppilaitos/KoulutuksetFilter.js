...
class KoulutuksetFilter extends Component {
  render() {
    ...
    return (
      <div
        className="col-12 col-lg-11 col-xl-12 col-xxxl-9"
        id="koulutukset-filter"
      >
        <div className="row">
          <div className="form-group" tabIndex="0">
            ...
            {filtersName.map((filter, i) => (
              <CheckBox filterName={filter} key={`${i}${filter}`} />
            ))}
          </div>
          <hr />
          ...
          <div className="form-group d-flex justify-content-around flex-column">
            <ActionButton
              type="transparent"
              text="Rajaa koulutuksia"
              className="align-self-center"
            />
            <a className="clear-filters align-self-center" href="/">
              Poista rajaukset
            </a>
          </div>
        </div>
