...
class ToteutusSidebar extends Component {
  ...
  render() {
    ...
    var tutkintonimikkeet =
      this.props.koulutus && this.props.koulutus.tutkintonimikes
        ? this.props.koulutus.tutkintonimikes
        : [];
    if (
      tutkintonimikkeet.length === 0 &&
      this.props.koulutus &&
      this.props.koulutus.tutkintonimike
    ) {
      tutkintonimikkeet.push(this.props.koulutus.tutkintonimike);
    }
    const hae =
      this.props.koulutus && this.props.koulutus.hakuajatFromBackend
        ? this.props.koulutus.hakuajatFromBackend.aktiiviset.length > 0
        : false;
    return (
      <div className="col-12 col-md-12 col-lg-4 col-xl-3 right-column">
        <div className="toteutus-sidebar">
          <div className="col-md-12">
            <h2 id="toteutus-side-header">
              {l.localize(this.props.organisaatio, '', 'Tuntematon oppilaitos')}
            </h2>
            <ActionButton type="react-link" address={link} />
          </div>
          {this.state.oppilaitos && (
            <div className="col-md-12">
              <h3>{t('toteutus.yhteystiedot')}</h3>
              <ContactInfoRow
                name={l.localize(this.props.organisaatio)}
                data={this.state.oppilaitos}
                type="toteutus"
          ...
          <div className="col-md-12">
            <dl>
              <dt>
                <h3>{t('toteutus.tutkintonimikkeet')}</h3>
              </dt>
              {tutkintonimikkeet.map((tn, i) => (
                <dd key={i}>
                  <span>{l.localize(tn, '')}</span>
                </dd>
              ))}
            </dl>
          </div>
          {hae && (
            <div className="col-md-12 hae">
              <span className="hae-koulutukseen">
                {t('toteutus.hae-koulutukseen')}
              </span>
            </div>
          )}
        </div>
