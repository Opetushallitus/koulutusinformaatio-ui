...
class Sivutus extends Component {
  ...
  createPaginationItem(page, active) {
    return (
      <li className={active ? 'active' : ''} key={page.toString()}>
        <a
          className="page-link"
          onClick={(e) => this.handlePagination(e, page)}
        >
          {page}
        </a>
      </li>
  ...
  createPaginationSeparator(key) {
    return (
      <li className={'item-separator'} key={key}>
        <a>...</a>
      </li>
    );
  }
  ...
  buildPaginationMenu(currentPage) {
    ...
    const paginationItems = currentOptions.map((o) =>
      this.createPaginationItem(o, o === currentPage)
    );
    ...
    if (maxPage > currentPage + 4) {
      paginationItems.splice(
        ...
        0,
        this.createPaginationSeparator('end')
      );
    }
    ...
    return (
      <React.Fragment>
        <div className="pagination-control container">
          {!this.props.hakuStore.filterSet &&
                ...
                <nav aria-label="Search result navigation" id="pagination">
                  <div className="row">
                    ...
                    <div className="page-size-select">
                      ...
                      >
                        <option value={this.props.hakuStore.pageSize}>
                          {t('haku.näytä')}
                        </option>
                        <option value={20}>20</option>
                        <option value={50}>50</option>
                        <option value={this.props.hakuStore.maxPageSize}>
                          {t('haku.kaikki')}
                        </option>
                      </select>
                    </div>
