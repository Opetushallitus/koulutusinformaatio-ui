...
class HakutulosSummary extends Component {
  ...
  renderFilterResultSummary() {
    const { t } = this.props;
    const koulutustyypit = this.props.hakuStore.filter.koulutus
      .map((k) => {
        switch (k) {
          case 'lk':
            return 'lukiot';
          case 'amm':
            return 'ammatilliset tutkinnot';
          case 'kk':
            return 'korkeakoulut';
          case 'yo':
            return 'korkeakoulut';
      ...
      .filter((k) => !!k)
      .join(', ');
    const kielet = this.props.hakuStore.filter.kieli
      .map((k) => {
        switch (k) {
    ...
    return (
      <div className="col-12 Etsinta">
        <h1 aria-live="assertive">
          {this.props.hakuStore.keywordSet
            ? t('ja')
            : t('haku.summary', { count: this.props.hakuStore.totalCount })}
          {koulutustyypit
            ? ' ' +
              t('haku.koulutustyypillä', {
                count: this.props.hakuStore.filter.koulutus.length,
              }) +
              ' '
            : ''}
          {koulutustyypit ? (
            <span className="highlight">{koulutustyypit}</span>
          ) : (
            ''
          )}
          {kielet
            ? (koulutustyypit ? ' ' + t('ja') : '') +
              (' ' +
                t('haku.kielellä', {
                  count: this.props.hakuStore.filter.kieli.length,
                })) +
              ' '
            : ''}
          {kielet ? <span className="highlight">{kielet}</span> : ''}
          {paikkakunta
            ? (koulutustyypit || kielet ? ' ' + t('sekä') : '') +
              (' ' + t('haku.paikkakunnalla') + ' ')
            : ''}
  ...
  render() {
    return (
      <div className="row result-count">
        {this.props.hakuStore.keywordSet
          ? this.renderKeywordResultSummary()
          : ''}
