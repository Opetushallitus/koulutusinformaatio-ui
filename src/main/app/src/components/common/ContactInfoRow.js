...
class ContactInfoRow extends Component {
  createContactData() {
    const data =
      this.props.data && this.props.data.yhteystiedot
        ? this.props.data.yhteystiedot
        : false;
    const contactData = {
      ...
      city:
        ...
        data.reduce((result, item) => {
          if (item.postitoimipaikka !== undefined) {
            result = `${item.postitoimipaikka
              .charAt(0)
              .toUpperCase()}${item.postitoimipaikka.slice(1).toLowerCase()}`;
          }
          return result;
        }, '---'),
      website:
            ...
            <div className="col-12 website">
              <div className="row">
                <h3>
                  ...
                  >
                    <span
                      className={`link-icon ${this.props.educationType}`}
                    ></span>
...
export default withTranslation()(ContactInfoRow);
