...
class HakutulosBox extends Component {
  ...
  handleKeyPress(e) {
    e.key === 'Enter' &&
      ...
      this.props.vertailuStore.selectItem(
        this.props.vertailuOid,
        this.props.nimi
      );
    ...
    return (
      <div
        ...
        className={`col-12 col-md-12 ${
          this.props.oppilaitos ? 'col-lg-12' : 'col-lg-6'
        } box-container`}
      >
        <div className={`col-12 search-box ${this.props.tyyppi}`}>
          {this.props.haettavissa && (
            <div className="haku">{this.props.t('haku.haku')}</div>
          )}
          <div className="d-flex justify-content-between">
            <div className="text">
              <Link to={this.props.link} className={'hakutulosbox-link'}>
                {this.props.nimi}
              </Link>
              <p>
                {this.props.text1}
          ...
          {this.props.vertailuStore.vertailuList.findIndex(
            (item) => item.oid === this.props.vertailuOid
          ) !== -1 ? (
            <div
              role="button"
              id="remove-compare"
            ...
            >
              <span className="poista">
                {this.props.t('haku.poista-vertailusta')}
              </span>
            </div>
          ) : this.props.vertailu ? (
            <div
