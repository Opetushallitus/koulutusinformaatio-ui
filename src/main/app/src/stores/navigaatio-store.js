...
class NavigaatioStore {
  ...
  @computed get hasNext() {
    return (
      this.index !== -1 &&
      (this.isLastOid ? !this.hakuStore.isLastPage : this.oids.length > 0)
    );
  }
  ...
  @computed get hasPrev() {
    return (
      this.index !== -1 &&
      (this.isFirstOid ? !this.hakuStore.isFirstPage : this.oids.length > 0)
    );
  }
  ...
  withPrevOid = (action) => {
    if (this.isFirstOid) {
      this.hakuStore.loadPrevPage(() => {
        action(this.oids[this.oids.length - 1]);
      });
    } else {
      action(this.oids[this.index - 1]);
    }
