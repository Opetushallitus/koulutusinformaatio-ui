...
class ContentfulStore {
    ...
    forwardTo = (id) => {
        return this.data.sivu[id] ? `/sivu/${this.data.sivu[id].slug || id}` : `/sivu/poistettu`;
    }
    ...
    fetchManifest() {
        return superagent.get(this.urlStore.urls.url('konfo-backend.content', 'manifest.json'));
    }
    fetchUrl(url) {
            ...
            .then(manifest => {
                const contents =
                    Object.entries(manifest)
                        .map(([k, v]) => [k, this.urlStore.urls.url('konfo-backend.content', '') + v[this.locale]]);
                return Promise.all(contents.map(([key, url]) => {
