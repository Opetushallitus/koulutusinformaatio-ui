...
beforeAll(async () => {
  ...
  page.on('request', (request) => {
    ...
    if (request.url().startsWith(backend + '/search/koulutukset?keyword=&')) {
      ...
      request
        .url()
        .startsWith(backend + '/search/koulutukset?keyword=datanomi&')
    ) {
      request.respond(response(Koulutukset.createKoulutuksetSearchResult(0)));
    } else if (
      request.url().startsWith(backend + '/search/oppilaitokset?keyword=&')
    ) {
...
describe('Etusivu', () => {
  ...
  it(
    ...
    async () => {
      ...
      expect(await page.$eval('.navbar-brand', (e) => e.innerHTML)).toBe(
        'Opintopolku'
      );
      expect(await page.$eval('.search-button', (e) => e.innerHTML)).toBe(
        'Etsi'
      );
  ...
  it(
    ...
    async () => {
      ...
      expect(await page.$eval('h1', (e) => e.innerHTML)).toMatch(
        new RegExp('Etsintäsi tuotti 0 osumaa.*')
      );
  ...
  it(
    ...
    async () => {
      ...
      expect(await page.$eval('h1', (e) => e.innerHTML)).toMatch(
        new RegExp('Etsintäsi tuotti 21 osumaa.*')
      );
      expect(await page.url()).toMatch(
        host +
          '/haku?toggle=koulutus&kpage=1&opage=1&pagesize=20&paikkakunta=kerava&lng=fi'
      );
  ...
  it(
    ...
    async () => {
      ...
      expect(await page.$eval('.palaute-form-header', (e) => e.innerHTML)).toBe(
        'Mitä mieltä olet uudesta Opintopolusta?'
      );
      expect((await page.$$('.icon-outline-star_border')).length).toEqual(5);
      expect(
        await page.$('.palaute-form-container .btn.inactive')
      ).toBeDefined();
